...
import { editorDataSelector, modeSelector, resetData, setMode, useGetArticleQuery, useGetDraftQuery } from '@/store';
...
const Editor = () => {
    ...
    const { isLoading, isError } = useGetArticleQuery(String(query.id) ?? '', {
        skip: mode !== 'editing',
        refetchOnMountOrArgChange: true,
    });
    const { isLoading: isLoadingDraft, isError: isErrorDraft } = useGetDraftQuery(String(query.id) ?? '', {
        skip: mode !== 'draft',
        refetchOnMountOrArgChange: true,
    });
