...
export const ArticleService = {
  ...
  async getArticle(slug: string) {
    const res = await fetch(`${env.NEXT_PUBLIC_STRAPI_API_URL}/articles/${slug}?populate=author&populate=cover`, {
      headers: {
  ...
  async getUserArticles(slug: string, authorId: number) {
    const res = await fetch(
      `${env.NEXT_PUBLIC_STRAPI_API_URL}/articles/?filters[slug][$ne]=${slug}&filters[author][id]=${authorId}&populate=author&populate=cover&pagination[pageSize]=4`,
      {
        headers: {
          Authorization: `bearer ${env.STRAPI_API_TOKEN}`
        },
  ...
  async getNextArticles(slug: string, authorId: number) {
    const res = await fetch(
      `${env.NEXT_PUBLIC_STRAPI_API_URL}/articles/?filters[slug][$ne]=${slug}&filters[author][id][$ne]=${authorId}&populate=author&populate=cover&pagination[pageSize]=4`,
      {
        headers: {
          Authorization: `bearer ${env.STRAPI_API_TOKEN}`
        },
  ...
  async getManyByUserLike() {
    ...
    const res = await fetch(`${env.NEXT_PUBLIC_STRAPI_API_URL}/articles/likes`, {
      headers: {
  ...
  async checkLike(articleId: number) {
    ...
    const res = await fetch(`${env.NEXT_PUBLIC_STRAPI_API_URL}/articles/${articleId}/likes`, {
      headers: {
